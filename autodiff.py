...
class MatMulOp(Op):
    ...
    def gradient(self, node, output_grad):
        ...
        return [self.__call__(output_grad, node.inputs[1], False, True),
    ...
    for node in reverse_topo_order:
        if node in node_to_output_grads_list:
            ...
            for id, in_nodes in enumerate(node.inputs):
                if not in_nodes in node_to_output_grads_list:
...
def test():
    ...
    print executor.run(feed_dict={x2: 2 * np.ones((3, 4))})
    #x2_val = 2 * np.ones(3)
